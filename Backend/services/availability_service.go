...
func (as *AvailabilityService) calculateAvailableSlots(
	...
	slots := make([]AvailableSlot, 0) // Initialize as empty slice, not nil
	...
	// If the latest booking time is before the start time, return empty array
	if slotEndTime.Before(currentTime) || slotEndTime.Equal(currentTime) {
		return slots
	}
