...
// Components
import FixedPriceBookingHeader from "../components/FixedPriceBookingHeader";
import BookingFilters from "../components/BookingFilters";
import QuoteModal from "../components/QuoteModal";

export default function FixedPriceBooking() {
  ...
  const handleWorkerRemoval = (booking: OptimizedBookingResponse) => {
    toast.info(`Remove worker from booking: ${booking.booking_reference}`);
  };
  ...
  const filteredBookings = bookings.filter((booking) => {
    // Filter by booking type (fixed price bookings only)
    if (booking.booking_type !== "regular") return false;
  ...
  const getStatusCounts = () => {
    ...
    // Only count fixed price bookings
    const fixedPriceBookings = bookings.filter(
      (booking) => booking.booking_type === "regular"
    );

    fixedPriceBookings.forEach((booking) => {
