...
export default function FixedPriceBooking() {
  ...
  const handleWorkerAssignment = (booking: OptimizedBookingResponse) => {
    toast.info(`Assign worker to booking: ${booking.booking_reference}`);
  };
  ...
  const filteredBookings = bookings.filter((booking) => {
    // Filter by booking type (regular bookings only)
    return booking.booking_type === "regular";
  });
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Manage all fixed price booking requests
              </p>
            </div>
