...
import {
  callMaskingApi,
  CloudShopeCallRequest,
  CloudShopeCallResponse,
} from "@/lib/callMaskingApi";

interface UseCallMaskingReturn {
  initiateCall: (
...
export function useCallMasking(): UseCallMaskingReturn {
  ...
  const initiateCall = async (
    ...
    try {
      const response = await callMaskingApi.initiateCloudShopeCall(request);
