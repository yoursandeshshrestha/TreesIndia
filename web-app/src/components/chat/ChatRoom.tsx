...
export const ChatRoom: React.FC<ChatRoomProps> = ({
  ...
  // Get mutations
  const { sendMessage, isSendingMessage, sendMessageError } =
    useChatMutations();
  ...
  if (!chatRoom && roomId && chatRoomData?.chat_rooms) {
    chatRoom = chatRoomData.chat_rooms.find((room: any) => room.id === roomId);
  }
  ...
  const { isConnected: isRealtimeConnected, isConnecting } = useWebSocket({
        ...
        <div className="p-4 border-b border-gray-200 bg-green-50">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              {(user as any)?.user_type === "worker"
                ? // For workers: show user info
                  (chatRoom.booking as any)?.user && (
                    <div className="flex items-center space-x-2">
                      <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                        {(chatRoom.booking as any).user.avatar ? (
                          <img
                            src={(chatRoom.booking as any).user.avatar}
                            alt={(chatRoom.booking as any).user.name}
                            className="w-8 h-8 rounded-full object-cover"
                          />
                        ) : (
                          <span className="text-sm font-medium text-gray-600">
                            {(chatRoom.booking as any).user.name
                              ?.charAt(0)
                              .toUpperCase() || "U"}
                          </span>
                        )}
                      ...
                      <span className="text-sm text-gray-700 font-medium">
                        {(chatRoom.booking as any).user.name || "User"}
                      </span>
                    </div>
                  )
                : // For users: show worker info
                  (chatRoom.booking as any)?.worker_assignment?.worker && (
                    <div className="flex items-center space-x-2">
                      <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                        {(chatRoom.booking as any).worker_assignment.worker
                          .avatar ? (
