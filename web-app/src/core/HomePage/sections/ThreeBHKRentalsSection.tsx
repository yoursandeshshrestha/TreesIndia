...
export default function ThreeBHKRentalsSection() {
  const router = useRouter();
  const { location, isLoading: locationLoading } = useLocation();
  const { isAuthenticated, user } = useAuth();
  const dispatch = useAppDispatch();

  // Create filters for 3 BHK rental properties
  ...
  // Use TanStack Query to fetch 3 BHK rental properties
  const {
    data: response,
    isLoading,
    error,
  ...
  const handleViewAllRentals = () => {
    router.push("/marketplace/rental-properties?bedrooms=3");
  };
  ...
  const getSectionTitle = () => {
    if (location?.city && location?.state) {
      return `3 BHK Rentals in ${location.city}`;
    }
    return "3 BHK Rentals";
  };
    ...
    <section className="px-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {properties.map((property, index) => (
          <PropertyCard
            key={`3bhk-rental-property-${property.ID}-${index}`}
            property={property}
