...
export default function TwoBHKRentalsSection() {
  const router = useRouter();
  const { location, isLoading: locationLoading } = useLocation();
  const { isAuthenticated, user } = useAuth();
  const dispatch = useAppDispatch();

  // Create filters for 2 BHK rental properties
  ...
  // Use TanStack Query to fetch 2 BHK rental properties
  const {
    data: response,
    isLoading,
    error,
  ...
  const handleViewAllRentals = () => {
    router.push("/marketplace/rental-properties?bedrooms=2");
  };
  ...
  const getSectionTitle = () => {
    if (location?.city && location?.state) {
      return `2 BHK Rentals in ${location.city}`;
    }
    return "2 BHK Rentals";
  };
    ...
    <section className="px-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {properties.map((property, index) => (
          <PropertyCard
            key={`2bhk-rental-property-${property.ID}-${index}`}
            property={property}
